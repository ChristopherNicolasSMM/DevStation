...
Todos os dados são serializados para dicts dentro da sessão (evita DetachedInstanceError)
"""
...
BADGE_COLOR = {
    ...
    "DEVELOPER": "indigo-6", "CORE_DEV": "purple-6",
    "DEV_ALL": "deep-orange-6", "ADMIN": "red-6",
}


def _serialize_profile(p):
    """Serializa Profile para dict enquanto sessão ainda está aberta."""
    return {
        'id':          p.id,
        'code':        p.code,
        ...
        'perm_ids':    [perm.id for perm in (p.permissions or [])],
        'perm_codes':  [perm.code for perm in (p.permissions or [])],
        'user_count':  len(p.users) if p.users is not None else 0,
    }


def _serialize_perm(p):
    return {
        'id':          p.id,
        'code':        p.code,
        'name':        p.name or p.code,
        'category':    p.category or 'Outros',
        'description': p.description or '',
        'is_system':   p.is_system,
    }


@create_page_layout("DS_PROFILES — Perfis de Acesso")
def render():
    user_data = get_current_user()
    if not user_data:
        return

    # ── DB helpers (retornam dicts, nunca ORM objects) ────────────────────
    def fetch_profiles():
        session = db_manager.get_session()
        try:
            return [_serialize_profile(p)
                    for p in session.query(Profile)
                    .order_by(Profile.priority.desc(), Profile.code).all()]
        finally:
            session.close()
    ...
    # ── Diálogos ──────────────────────────────────────────────────────────
    def open_profile_dialog(profile_dict=None):
        is_edit   = profile_dict is not None
        all_perms = fetch_permissions()
        cats      = perm_by_cat(all_perms)
        cur_pids  = set(profile_dict['perm_ids']) if is_edit else set()

        with ui.dialog() as dlg, \
             ui.card().classes('w-[640px] bg-[#161b22] border border-[#30363d] p-6'):
            ...
            with ui.tabs().classes('w-full') as tabs:
                t_info  = ui.tab('Dados',       icon='info')
                t_perms = ui.tab('Permissões',  icon='lock')

            with ui.tab_panels(tabs, value=t_info).classes('w-full pt-3'):

                with ui.tab_panel(t_info):
                    with ui.column().classes('gap-3 w-full'):
                        f_code = ui.input('Código *',
                                          value=profile_dict['code'] if is_edit else '') \
                            .classes('w-full').props('outlined dense dark color=blue-4')
                        f_name = ui.input('Nome *',
                                          value=profile_dict['name'] if is_edit else '') \
                            .classes('w-full').props('outlined dense dark color=blue-4')
                        f_desc = ui.input('Descrição',
                                          value=profile_dict['description'] if is_edit else '') \
                            .classes('w-full').props('outlined dense dark color=blue-4')
                        f_prio = ui.number('Nível / Prioridade',
                                           value=profile_dict['priority'] if is_edit else 0,
                                           min=0, max=99) \
                            .classes('w-48').props('outlined dense dark color=blue-4')
                        with ui.row().classes('gap-6'):
                            f_active = ui.checkbox('Ativo',
                                                   value=profile_dict['is_active'] if is_edit else True)
                            f_system = ui.checkbox('Sistema',
                                                   value=profile_dict['is_system'] if is_edit else False)

                with ui.tab_panel(t_perms):
                    ui.label('Marque as permissões concedidas:') \
                    ...
                    with ui.scroll_area().classes('h-72 w-full border border-[#30363d] rounded p-2'):
                        for cat_name, cat_perms in cats.items():
                            ui.label(cat_name.upper()) \
                                .classes('text-[#8b949e] text-xs font-bold mt-2 mb-1')
                            for perm in cat_perms:
                                cb = ui.checkbox(perm['code'],
                                                 value=perm['id'] in cur_pids) \
                                    .classes('text-xs')
                                with ui.tooltip():
                                    ui.label(perm['description'] or perm['name'])
            ...
            def save():
                ...
                sel_ids = [pid for pid, cb in perm_checks.items() if cb.value]
                session = db_manager.get_session()
                try:
                    if is_edit:
                        p = session.query(Profile).filter(
                            Profile.id == profile_dict['id']).first()
                    else:
                    ...
                    p.permissions = session.query(Permission) \
                        .filter(Permission.id.in_(sel_ids)).all()
                    session.commit()
    ...
    def open_delete_dialog(profile_dict):
        with ui.dialog() as dlg, \
            ...
            with ui.row().classes(
                ...
                ui.label(f'Excluir perfil "{profile_dict["code"]}"?') \
                    .classes('text-[#c9d1d9] text-sm')

            def do_delete():
                if profile_dict['is_system']:
                    ui.notify('Perfis do sistema não podem ser excluídos.', type='warning')
                    return
                session = db_manager.get_session()
                try:
                    p = session.query(Profile).filter(
    ...
    with ui.row().classes('items-center justify-between w-full mb-4'):
        ...
        ui.button('Novo Perfil', icon='add_circle',
    ...
    def refresh():
        ...
        with content_col:
            ...
            with data_card('Hierarquia de Perfis'):
                p_map = {p['code']: p for p in profiles}
                steps = [c for c in HIERARCHY_ORDER if c in p_map] + \
                ...
                with ui.row().classes('gap-0 items-stretch flex-wrap'):
                    for i, code in enumerate(steps):
                        p = p_map.get(code)
                        color = BADGE_COLOR.get(code, 'grey-6')
                        with ui.column().classes('items-center'):
                            with ui.card().classes(
                                'border border-[#30363d] bg-[#0d1117] px-4 py-3 m-1 min-w-[110px] items-center'
                            ):
                                ui.badge(code).props(f'color={color}').classes('mb-1')
                                ui.label(p['name'] if p else code) \
                                    .classes('text-white text-xs text-center')
                                with ui.row().classes('gap-2 mt-1'):
                                    n_p = len(p['perm_codes']) if p else 0
                                    n_u = p['user_count'] if p else 0
                                    ui.label(f'{n_p}p').classes('text-[#8b949e] text-xs')
                                    ui.label(f'{n_u}u').classes('text-[#8b949e] text-xs')
                        if i < len(steps) - 1:
            ...
            with ui.row().classes('gap-4 flex-wrap'):
                for pd in profiles:
                    color  = BADGE_COLOR.get(pd['code'], 'grey-6')
                    pcount = len(pd['perm_codes'])
                    cats_d = {}
                    for perm in all_perms:
                        if perm['id'] in pd['perm_ids']:
                            cats_d.setdefault(perm['category'], []).append(perm['code'])

                    with ui.card().classes(
                        'bg-[#161b22] border border-[#30363d] p-5 w-80'
                    ):
                        with ui.row().classes('items-start justify-between mb-3'):
                            with ui.column().classes('gap-1'):
                                ui.badge(pd['code']).props(f'color={color}')
                                ui.label(pd['name']).classes('text-white text-sm font-semibold mt-1')
                                if pd['description']:
                                    ui.label(pd['description']) \
                                        .classes('text-[#8b949e] text-xs')
                            with ui.row().classes('gap-1'):
                                ui.button(
                                    icon='edit',
                                    on_click=lambda d=pd: open_profile_dialog(d)
                                ).props('flat dense color=blue-4')
                                ui.button(
                                    icon='delete',
                                    on_click=lambda d=pd: open_delete_dialog(d)
                        ...
                        with ui.row().classes('gap-4 mb-2'):
                            for val, lbl, cls in [
                                (pcount,          'permissões', 'text-blue-400'),
                                (pd['user_count'],'usuários',   'text-green-400'),
                                (pd['priority'],  'nível',      'text-white'),
                            ]:
                                with ui.column().classes('items-center gap-0'):
                                    ui.label(str(val)).classes(f'{cls} text-lg font-bold')
                                    ui.label(lbl).classes('text-[#8b949e] text-xs')

                        if cats_d:
                            with ui.expansion('Permissões', icon='lock').classes('w-full text-xs'):
                                for cat_name, codes in cats_d.items():
                                    ui.label(cat_name).classes(
                                        'text-[#8b949e] text-xs uppercase font-bold mt-1')
                                    for code in codes:
                                        with ui.row().classes('items-center gap-1'):
                                            ui.icon('check').classes('text-green-400 text-xs')
                                            ui.label(code).classes('text-white text-xs font-mono')
            ...
            cats_all = perm_by_cat(all_perms)
            if cats_all:
                with data_card('Todas as Permissões do Sistema'):
                    with ui.tabs().classes('w-full') as ptabs:
                        cat_tab_map = {c: ui.tab(c) for c in cats_all}

                    first_tab = list(cat_tab_map.values())[0] if cat_tab_map else None
                    with ui.tab_panels(ptabs, value=first_tab).classes('w-full'):
                        for cat_name, ctab in cat_tab_map.items():
                            with ui.tab_panel(ctab):
                                pcols = [
                                ...
                                prows = []
                                for perm in cats_all.get(cat_name, []):
                                    prof_codes = [
                                        pd['code'] for pd in profiles
                                        if perm['id'] in pd['perm_ids']
                                    ]
                                    prows.append({
                                        'code':        perm['code'],
